...
rm(list=ls())
dev.off()##borrado de gráficos
library(cutoffR)
# Ingresar una base de datos en csv: %b-%y (date), datos mensuales (cabecera con codigo de estaciones)
data <- read.csv("E:/curso_R/4_CompletarDatos/16_datam.csv",header=TRUE, check.names = F, stringsAsFactors = F)
# Ingresar la ruta y el nombre del archivo de salida
output <- "E:/curso_R/4_CompletarDatos/completm.csv"
